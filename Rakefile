...
RestKit::Rake::ServerTask.new do |t|
  t.port = 4567
  t.pid_file = 'TransitTests/Server/server.pid'
  t.rackup_file = 'TransitTests/Server/server.ru'
  t.log_file = 'TransitTests/Server/server.log'

  t.adapter(:thin) do |thin|
    thin.config_file = 'TransitTests/Server/thin.yml'
  end
end
...
namespace :test do
  ...
  task :application => :kill_simulator do
    config = Xcode.project(:Transit).target(:TransitTests).config(:Debug)
